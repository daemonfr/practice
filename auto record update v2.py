...
def check_inbox(mail_box):
    ...
    global list_of_ppl_completed_form
    ...
    for j, k in enumerate(messages):
        ...
        if pattern.findall(k.Subject):
            ...
            num_of_ppl_completed_form += 1
            found = False
            if txt.findall(k.Body):
                list_of_ppl_completed_form.append(txt.findall(k.Body)[0])
                found = True
            if not found:
                print(timestamp() + ': No name found for email ' + str(count) + ' although subject matches.')
