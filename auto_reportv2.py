...
class GenerateReport:
    def __init__(self, path, in_file, sheet, start_col, days_in_month, year_start_col, status, offices, report_items):
        self.path = path
        self.in_file = in_file
        self.sheet = sheet
        self.start_col = start_col
        self.days_in_month = days_in_month
        self.year_start_col = year_start_col
        self.status = status
    ...
    def inputFile(self):
        os.chdir(self.path)
        end_col = self.start_col + self.days_in_month
        df1 = pd.read_excel(self.in_file, sheet_name=self.sheet)

        col_range = list(range(self.start_col, end_col))
    ...
    def yeartoMonth(self):
        ...
        year_end_col = self.start_col - 1
        ytm_col_range = list(range(self.year_start_col, year_end_col))
...
initiate = GenerateReport(path, in_file, sheet, start_col, days_in_month, year_start_col, status, offices, report_items)
